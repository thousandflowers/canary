...
// binaryPath is the absolute path to this executable, symlink and all.
//
...
//
// Symlink and all, because Homebrew's bin/canary points into a version-pinned
// Cellar directory that the next `brew cleanup` deletes. The shell hook is
// re-generated by `canary init` on every shell start and would heal itself, but
// the status line command is written into settings.json once: resolving the
// link there buys a path that stops existing at the next upgrade, and a status
// line that silently goes blank.
