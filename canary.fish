...
set -g _CANARY_NIGHT_START 22
set -g _CANARY_NIGHT_END 7
...
function _canary_compute
    ...
    set -l count $CANARY_PROMPT_COUNT
    set -l avglen (_canary_avg)
    set -l score (math "floor($min / 3 + $count / 2 + $avglen / 10)")

    set -l hour (date +%H | sed 's/^0*//')
    test -z "$hour"; and set hour 0
    if test $hour -ge $_CANARY_NIGHT_START -o $hour -lt $_CANARY_NIGHT_END
        set score (math "floor($score * 13 / 10)")
    end

    test $score -gt 100; and set score 100
